...
func New(cfg *config.Config) (*Server, error) {
	...
	if cfg.TenantsFile != "" {
		if err := tenantMgr.LoadFromFile(); err != nil {
			...
			cfg.UpdateDataDirFromTenants(tenantMgr)
		}
	}
	...
	if cfg.TenantsFile != "" && tenantMgr != nil {
		storageBackend = storage.NewTenantAwareStorage(cfg.DataDir, tenantMgr, cfg.InMemory)
	} else if cfg.InMemory {
		storageBackend = storage.NewMemoryStorage()
	} else {
		storageBackend, err = storage.NewFileSystemStorage(cfg.DataDir)
		if err != nil {
...
func (s *Server) getStorageType() string {
	...
	if _, ok := s.storage.(*storage.TenantAwareStorage); ok {
		...
		return fmt.Sprintf("tenant-aware filesystem (base: %s)", s.config.DataDir)
	}

	if s.config.InMemory {
		return "in-memory"
	}
	return fmt.Sprintf("filesystem (%s)", s.config.DataDir)
}
